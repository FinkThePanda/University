...
$Revision: 45 $
$Id: mona.py 45 2011-07-02 22:33:59Z corelanc0d3r $ 
"""
...
def getSkeletonHeader(exploittype,portnr,extension):
	skeletonheader =  "##\n"
	skeletonheader += "# $" + "Id" + ": $\n"
	skeletonheader += "# Skeleton generated by mona.py - Corelan Team\n"
	skeletonheader += "##\n\n"
	skeletonheader += "##\n"
	...
	skeletoninit += "\t\t\t'Name'\t\t=> '<insert name for the exploit>',\n"
	skeletoninit += "\t\t\t'Description'\t=> %q{\n"
	skeletoninit += "\t\t\t\t\tThis module exploits... <insert details about the vulnerability>\n"
	skeletoninit += "\t\t\t},\n"
	skeletoninit += "\t\t\t'License'\t\t=> MSF_LICENSE,\n"
	skeletoninit += "\t\t\t'Author'\t\t=>\n"
	skeletoninit += "\t\t\t\t[\n"
	skeletoninit += "\t\t\t\t\t'<insert name of person who discovered the vulnerability>',\t# Original discovery\n"
	skeletoninit += "\t\t\t\t\t'<insert your name here>',\t# MSF Module\n"
	skeletoninit += "\t\t\t\t\t'Corelan Team',\t# tx for mona.py :)\n"			
	skeletoninit += "\t\t\t\t],\n"
	skeletoninit += "\t\t\t'Version'\t\t=> '$" + "Revision:$',\n"
	skeletoninit += "\t\t\t'References'\t=>\n"
	skeletoninit += "\t\t\t\t[\n"
	skeletoninit += "\t\t\t\t\t[ 'OSVDB', '<insert OSVDB number here>' ],\n"
	skeletoninit += "\t\t\t\t\t[ 'CVE', '<insert CVE number here>' ],\n"
	skeletoninit += "\t\t\t\t\t[ 'URL', '<insert another link to the exploit/advisory here>' ]\n"
	skeletoninit += "\t\t\t\t],\n"
	skeletoninit += "\t\t\t'DefaultOptions' =>\n"
	skeletoninit += "\t\t\t\t{\n"
	skeletoninit += "\t\t\t\t\t'EXITFUNC' => 'process',\n"
	skeletoninit += "\t\t\t\t},\n"
	skeletoninit += "\t\t\t'Platform'\t=> 'win',\n"
	skeletoninit += "\t\t\t'Payload'\t=>\n"
	skeletoninit += "\t\t\t\t{\n"
	skeletoninit += "\t\t\t\t\t'BadChars' => \"\\x00\", # <change if needed>\n"
	skeletoninit += "\t\t\t\t},\n"
...
def findROPGADGETS(modulecriteria={},criteria={},endings=[],maxoffset=40,depth=5,split=False,pivotdistance=0,fast=False,mode="all"):
	...
	if mode == "all":
		...
		objprogressfile.write(updatetext.strip(),progressfile)
		vplogtxt = createRopChain(suggestions,interestinggadgets,modulecriteria,criteria)
...
def findPattern(modulecriteria,criteria,pattern,type,base,top,consecutive,rangep2p=0):
	...
	if "p2p" in criteria and len(allpointers) > 0:
		if not silent:
			imm.log("[+] %d locations found" % len(allpointers))
		...
		if pushad and pickup:
			...
			if shortest_pickup == 0: 	#no esi
					...
					if pickuptypes.find("move") > -1 and pickuptypes.find("-> esi") > -1 and not pickupfound:
						...
						if "pickup pointer into "+thisreg in suggestions:
							...
							if popptr > 0:
								...
								prepickuptxt += "\t\t\t0x" + ropfuncptr + ",\t# <- *&VirtualProtect() \n"
				...
				if not pickupfound:
					#show what we've got
					for pickuptypes in suggestions:
						if pickuptypes.find("pickup pointer into") > -1:
							shortest_pickup = getShortestGadget(suggestions[pickuptypes])
							pickupstr = interestinggadgets[shortest_pickup].strip()
							pickupstrparts = pickupstr.split("[")
							pickupstrsubparts = pickupstrparts[1].split("]")
							pickupfrom = pickupstrsubparts[0]
							
							pickupstrsubparts2 = pickupstr.split(",")
							pickupstrsubparts3 = pickupstrsubparts2[0].split(" ")
							pickupto = pickupstrsubparts3[2]
							if "pop "+pickupfrom.lower() in suggestions:
								popptr = getShortestGadget(suggestions["pop "+pickupfrom.lower()])
							else:
								popptr = 0
							if popptr > 0:
								...
								lastreturnoffset = getOffset(interestinggadgets[shortest_pickup])
								pickuptxt += createJunk(getJunk(interestinggadgets[shortest_pickup])-4)
								postpickuptxt += "\t\t\t0x????????,\t# ** <- find routine to move virtualprotect() into esi\n"								
								postpickuptxt += "\t\t\t           \t# ** Hint : look for mov [esp+offset]," + pickupto.lower() + " and pop esi\n"
								pickupfound = True
								break
			...
			if not pickupfound:
				vplogtxt += "\t\t\t#couldn't find rop gadgets to pickup virtualprotect into esi\n"
			else:
			...
			if len(jmpesp) > 0:
				if "pop ebp" in suggestions:	# and popptr > 0:
					#get the shortest one
					...
					preesptxt += createJunk(getJunk(interestinggadgets[popebpptr])-4)
...
def getRopSuggestion(ropchains):
	...
	pushad_allowed = [ "INC ","DEC ","OR ","XOR ","LEA ","ADD ","SUB ", "PUSHAD", "RETN ", "NOP", "POP ","PUSH EAX","PUSH EDI","ADC ","FPATAN" ]
	for r in regs:
	...
	for reg in regs:
		...
		moveptr_notallowed = ["POP ESP","MOV ESP,","XCHG ESP,","XOR ESP","LEA ESP,","AND ESP", "ADD ESP", "MOV DWORD PTR","ADD DWORD PTR","LEA DWORD PTR","SUB ESP","OR ESP"]
	...
	# =====
	for reg in regs:
		empty_allowed = ["XOR "+reg+","+reg+" # RETN","MOV "+reg+",FFFFFFFF # INC "+reg+" # RETN", "SUB "+reg+","+reg+" # RETN", "PUSH 0 # POP "+reg + " # RETN"]
		empty_notallowed = []
		for gadget in ropchains:
			gadgetinstructions = ropchains[gadget].strip()
...
# ----- offset: Calculate the offset between two addresses ----- #
		...
		def procSuggest(args):
			...
			skeletonheader,skeletoninit,skeletoninit2 = getSkeletonHeader(exploittype,portnr,extension)
				
			if isEIP:
				imm.log("[+] Attempting to create payload for saved EIP overwrite...")
